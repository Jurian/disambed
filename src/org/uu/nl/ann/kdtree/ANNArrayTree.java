...
import org.uu.nl.ann.kdtree.split.SplitPolicy;
import org.uu.nl.ann.kdtree.util.Distance;
import org.uu.nl.ann.kdtree.util.HyperRectangle;
import org.uu.nl.ann.kdtree.util.LimitedPriorityQueue;
...
public class ANNArrayTree<T> implements ANN<T> {
	...
	public static final class TreeNode<V> {
		...
		public TreeNode(int splitDimension, double[] keyVector, V val) {
	...
	public void add(double[] vec, T value) {
		...
		if (root == null) {
			root = new TreeNode<T>(splitRule.splitDimension(vec, 0, treeNodes), vec, value);
			return;
		}
		...
		TreeNode<T> n = new TreeNode<T>(splitDimension, vec, value);
		treeNodes.add(n);
		if (right) {
			current.right = n;
		} else {
	...
	public void balance() {
		...
		Collections.sort(treeNodes,
				(o1, o2) -> Double.compare(o1.keyVector[o1.splitDimension], o2.keyVector[o2.splitDimension]));
	...
	private void getNearestNeighbourInternal(TreeNode<T> current, double[] target, HyperRectangle hyperRectangle,
		...
		leftHyperRectangle.max[s] = pivot[s];
