...
public class SimpleBookmarkColoring implements CooccurenceMatrix {
	...
	public SimpleBookmarkColoring(Model model, Options options, Publisher publisher) {
		...
		try {
			...
			for(Node bookmark : stats.jobs) {
				completionService.submit(new SimpleBCAJob(bookmark, includeReverse, normalize, alpha, epsilon, stats.keys, model));
			}
