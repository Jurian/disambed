...
/**
 * Interface class for logic d-DNNF clauses 
 * to be used in d-DNNF.
 * 
 ...
 * @since 11-06-2020
 */
public class DdnnfClause implements DdnnfLogicRule {
    ...
    private Set<String> positiveNameSet = new TreeSet<>();
    /**
	 ...
	 * is generated from.
	 */
	public DdnnfClause(final DdnnfLogicRule[] orderedRules, final boolean[] orderedNegated, final CnfLogicRule sourceRule, final String name) {
		super();
	...
	public DdnnfClause(final DdnnfLogicRule[] orderedRules, final boolean[] orderedNegated, final String name) {
		super();
		if(!(orderedRules.length == orderedNegated.length)) {
			throw new IllegalArgumentException("The two arrays do not have the same length."); }
		
		for(int i = 0; i < orderedRules.length; i++) {
			if(orderedRules[i] instanceof DdnnfLiteral) {
				if(orderedNegated[i]) { addPositiveLiteral((DdnnfLiteral) orderedRules[i]); }
				else { addNegativeLiteral((DdnnfLiteral) orderedRules[i]); }
			}
			else if((orderedRules[i] instanceof DdnnfClause) || (orderedRules[i] instanceof DdnnfFormula)) {
	...
	/*
	public DdnnfClause(final CnfLogicRule sourceRule) {
		super();
	 ...
	 *
	private void convertCnfClause(final Clause clause) {
		// Initialize temporary sets.
	    Set<DdnnfLiteral> posLits = new TreeSet<>();
	    Set<DdnnfLiteral> negLits = new TreeSet<>();
	    Set<String> posNames = new TreeSet<>();
	    Set<String> negNames = new TreeSet<>();
	    ...
	    // add them to the sets and ArrayList.
	    for(LogicLiteral lit : clause.getPositiveLiterals()) {
	    	literal = new DdnnfLiteral(lit.getName(), lit.getAssignment(), lit.isNegated(), lit);
	    	allLits.add(literal);
	    	posLits.add(literal);
	    	posNames.add(lit.getName());
	    }
	    for(LogicLiteral lit : clause.getNegativeLiterals()) {
	    	literal = new DdnnfLiteral(lit.getName(), lit.getAssignment(), lit.isNegated(), lit);
	    	allLits.add(literal);
	    	negLits.add(literal);
	    	negNames.add(lit.getName());
	    }
	    ...
	    for(int i = 0; i < allLits.size(); i++) {
	    	...
	    	// Add the newly formed d-DNNF formula to the rules of this clause.
	    	rules.add(new DdnnfFormula(litArray, getsNegVal, clause));
	    }
	...
	private void convertCnfFormula(final CnfFormula formula) {
		...
		for(int i = 0; i < clauseArr.length; i++) {
			for(int j = i+1; j < clauseArr.length; j++) {
				...
				if(!Collections.disjoint(clauseArr[i].getLiterals(), clauseArr[j].getLiterals())) {
					...
					this.rules.addAll(resolveFormulaDuplicates(clauseArr[i], clauseArr[j], formula));
				}
			}
		}
		
		DdnnfLiteral[] posLiterals, negLiterals, literals;
		...
		for(int i = 0; i < clauseArr.length; i++) {
			if(clauseNoDuplicate[i]) {
				posLiterals = clauseArr[i].getPositiveLiterals().toArray(new DdnnfLiteral[0]);
				areNegatedPos = new boolean[posLiterals.length];
				negLiterals = clauseArr[i].getNegativeLiterals().toArray(new DdnnfLiteral[0]);
     ...
     */
    @Override
    public ArrayList<DdnnfLogicRule> getRules() {
    	return this.rules;
    }
    ...
    public boolean equals(Object obj) {
        ...
        return this.getName() == ((DdnnfClause) obj).getName();
    }
