...
public class RuleGradient {
	 ...
	 */
	public void calculateGradientGlove(double weight) throws Exception {
		...
		// If third Triple is null.
		if (this.Rule.getThirdTriple() == null){
			this.dFstPi = 0.0;
			double dPosPi = 0.0, tripleEmbedding;
			for (int p = 0; p < iNumberOfFactors; p++) {
				 ...
				 */
				tripleEmbedding = MatrixE.get(iFstHead, p) + MatrixR.get(iFstRelation, p) - MatrixE.get(iFstTail, p);
				this.dFstPi -= Math.abs(tripleEmbedding);
			}
			dFstPi *= dValue;
			dFstPi += 1.0;
